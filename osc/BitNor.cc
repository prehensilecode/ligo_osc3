// Yo, Emacs! This is -*- C++ -*-
//
// $Id: BitNor.cc,v 1.4 2002/06/27 18:27:00 dwchin Exp $
//
...
bool osc::BitNor::satisfied()
{
    if (mDebug > 0) {
        cout << "osc::BitNor on '" << mChannel
    ...
    // doesn't change from frame to frame, so we will use a
    ...
    if (t0 == mCurResults.mT0) {
        ...
        // if error, give up
        if (nRead != nSample) {
            cerr << "osc::bitnor: error reading data from channel '"
