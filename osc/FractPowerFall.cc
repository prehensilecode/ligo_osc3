// Yo, Emacs! This is -*- C++ -*-
//
// $Id: FractPowerFall.cc,v 1.2 2002/05/01 18:47:43 dwchin Exp $
//
...
osc::FractPowerFall::FractPowerFall(Dacc           *access,
                                ostream        *outstream,
                                std::string     chanName,
                                osc::IntParMap &intparams,
                                osc::DblParMap &dblparams,
                                int             debug)
    : OperStateCond(access, outstream, chanName, "FractPowerFall",
...
double osc::FractPowerFall::powerFract(const TSeries *ptx,
                                       double         freqlo,
                                       double         freqhi)
{
...
bool osc::FractPowerFall::satisfied()
{
    if (mDebug > 0) {
    ...
    if (t0 == mCurResults.mT0) {
        ...
        if (mDebug > 0) {
            cout << "osc::fractpowerfall::satisfied(): "
        ...
        mCurResults.mDoubleVal = *(mPowerHist.end() - 1) - *(mPowerHist.begin());
